...
*/
let me = { name: "marco", surname: "zagaria", age: 28 };
console.log(me);
/* ESERCIZIO D
...
*/
const deleteProp = (ogg, string) => {
  delete ogg[string];
  return ogg;
};
console.log(deleteProp({ a: "ciao", b: "tuttu" }, "a"));
...
*/
const newestMovie = (n) => {
  let annoMinimo = { Year: 1990 };
  n.forEach((movies) => {
    let filmCorrente = parseInt(movies.Year);
    if (filmCorrente > annoMinimo.Year) {
      annoMinimo = movies;
    }
  });
  return annoMinimo;
...
const searchAndDivide = (string) => {
  ...
  movies.forEach((movies) => {
    if (movies.Title.includes(string)) {
      object.match.push(movies);
    } else {
      object.unmatch.push(movies);
    }
  });
  return object;
};
...
*/
const colora = () => {
  let links = document.getElementsByClassName("color");
  for (let index = 0; index < links.length; index++) {
    links[index].classList.add("class");
  }
};
colora();
/* ESERCIZIO 24
